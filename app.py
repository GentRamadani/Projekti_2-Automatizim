...
st.write("""
This chart displays the **top 10 products ranked by Revenue per Unit**, which is calculated by dividing the total revenue generated by a product by the total quantity sold.

Unlike total revenue, this metric highlights products that generate the highest value for each individual unit sold. It helps identify premium or high-value products that may contribute significantly to profitability even if they are sold in relatively small quantities.

**Formula used:**

`Revenue per Unit = Total Revenue ÷ Total Quantity Sold`

Only products with sufficient sales volume are included to avoid misleading results caused by products sold in very small quantities.
""")
